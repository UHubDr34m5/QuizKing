...
- `Questions`: 問題、形式、正解、解説、難易度、表示順、任意の画像URL
...
既存のGASデプロイとの互換性を保つため、現在のスプレッドシートでは `type` を `text`、問題文の先頭を `【多答】` とし、`choicesJson` に正解項目のJSON配列を入れても多答として認識します。順不同で全項目が一致したときに正解です。別名を許可するときは、1項目を `正規表記::別名1::別名2` の形式にします。

例：`["1964年 東京","1972年 札幌","1998年 長野","2020年 東京"]`

更新後の `gas/Code.gs` を再デプロイした環境では、管理画面から `multi` を選んで最大100項目まで登録できます。
...
例：問題文を `「進捗」は何と読む？`、正解を `しんちょく` として公開すると、通常クイズと漢字キングの両方に自動で追加されます。漢字の意味・部首・書き取りなど、読みを尋ねない漢字問題は漢字キングには入りません。
